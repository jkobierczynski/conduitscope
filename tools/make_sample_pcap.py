...
def build_s7comm_sample():
    ...
    # 5) S7comm Read Var -- Job request (item descriptor bytes are dummy; not decoded by conduitscope)
    read_param = bytes([0x04, 0x01]) + bytes(range(12))
    ...
    # 6) S7comm Read Var -- Ack_Data response, with a small data block
    read_resp_param = bytes([0x04, 0x01])
    read_resp_data = bytes([0xFF, 0x04, 0x00, 0x20]) + struct.pack("!i", 42)
