...
// files). `decode` (and, as a normal consequence of protocol dispatch, `policy validate`/
// `inventory` already) has recognized all 42 of these protocol names by their own `DecodedPacket::
// protocol` value since Tiers 1-5 were completed -- see docs/DEVELOPMENT.md's ROADMAP item 18. What
// this file adds is the second half of that same item: a single place PolicyEngine::observe
// (policy_engine.cpp) and AssetInventoryEngine::observe (asset_inventory.cpp) both call to ask "is
// this one of the 42 protocols an OT auditor flags, and if so which tier?" -- so `policy validate`
// and `inventory` can each surface every one of these protocols' mere PRESENCE as its own finding,
// independent of whatever a conduit's own allow-list decides about it (see PolicyReport::
// notable_protocols' and AssetInventoryReport::notable_protocols' own comments for exactly how that
// finding is aggregated and reported).
//
// Deliberately just a name -> tier lookup, nothing more: every one of these 42 protocol values is
// already fully named by decode's own dispatch (it_protocols.hpp/tunnel_vpn.hpp/eapol.hpp/pppoe.hpp/
// mpls.hpp) before a DecodedPacket ever reaches this file, so there is no decoding work here, only
// the tier classification those five ROADMAP item 18 paragraphs already establish in prose.
#pragma once
...
// zoom), "lateral-movement" (Tier 2: smb/ssh/http/https/snmp/telnet/ftp/tftp), "enterprise-trust"
...
// `protocol` is one of these 42 values `DecodedPacket::protocol` can be set to (see decoder.hpp's
