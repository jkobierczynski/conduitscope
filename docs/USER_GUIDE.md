...
handshake state for these 43 protocols to draw a confirmed direction from,
...
(none of these 43 ports are ever "known" the way Modbus/DNP3/S7comm/IEC104/
...
command has no equivalent per-session handshake state for these 43
...
- `ffhse_body_length` / `ffhse_body_hex`: present only when
  ...
  hex rendering.
- `twincat_command`: the Command ID's canonical name (e.g. `"Read"`,
  `"AddDeviceNotification"`), always present when `protocol` is
  `twincat`.
- `twincat_is_response`: `true`/`false` (State Flags bit 0), always
  present when `protocol` is `twincat`.
- `twincat_invoke_id`: the 4-byte Invoke ID used for request/response
  pairing, always present when `protocol` is `twincat`.
- `twincat_target_ams_net_id` / `twincat_target_ams_port` /
  `twincat_source_ams_net_id` / `twincat_source_ams_port`: the AMS header's
  addressing fields, always present when `protocol` is `twincat` --
  `*_ams_net_id` rendered dotted-decimal like an IPv4 address with two
  extra octets (e.g. `"5.62.196.212.1.1"`), never confused with `src_ip`/
  `dst_ip` (the underlying TCP/IP addressing, still populated as usual).
- `twincat_error_code`: the AMS header's own Error Code field, present
  only when nonzero.
- `twincat_index_group` / `twincat_index_offset`: the PLC symbol/variable
  address (IndexGroup/IndexOffset), present only for Read/Write/ReadWrite/
  AddDeviceNotification, which carry this addressing.
- `twincat_ads_result`: the payload's own ADS Result code, present only on
  a response carrying one -- every recognized response shape except
  DeviceNotification, which carries no Result at all (it's an unsolicited
  push, not a reply to a specific request).
- `twincat_paired_request_index`: the 1-based packet index of this
  response's matched request, present only on an authoritatively paired
  response (Invoke ID + AMS/TCP session match, non-heuristic -- the same
  authoritative pairing `modbus_paired_request_index` provides for
  Modbus). See docs/PROTOCOL_COVERAGE.md's TwinCAT / ADS section.
...
- **IGRP, PIM, EIGRP, and OSPF have no real-capture validation at all** --
  ...
  account.
- **BGP has no authentication visibility at all -- not merely unverified,
  genuinely invisible.** Unlike RIP's/EIGRP's/OSPF's own "digest present but
  unverified" posture just above (those protocols at least locate an
  authentication field and show that it's there), real-world BGP
  authentication (RFC 2385 TCP-MD5, RFC 5925 TCP-AO) lives entirely outside
  the BGP message body, at the TCP layer -- there is nothing for a passive
  decoder to even locate. A decodable BGP session says nothing at all about
  whether it's authenticated. **BGP is not wired into the `policy validate`
  conduit `protocols` classification either**, the same gap as every routing
  protocol above. **BGP has no real-capture validation** -- validated by
  construction only, against synthetic `tests/sample_bgp.pcap`. See
  docs/PROTOCOL_COVERAGE.md's BGP-4 section.
